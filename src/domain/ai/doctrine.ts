/**
 * Doctrines: the weights that turn the evaluator's numbers into a preference. They encode the
 * tensions of RULES.md §24 — the wedge protects but cripples the active defenses, pivoting
 * forfeits displacement, closing to beam range crosses the enemy's best missile envelope, and
 * whoever controls the range controls the salvo count.
 *
 * Damage terms are in tens of damage points, so a weight of 1 is worth about ten points.
 */
import type { Doctrine } from '../game';

export interface DoctrineWeights {
  /** Multiplier on expected damage dealt. */
  readonly offense: number;
  /** Multiplier on expected damage received. */
